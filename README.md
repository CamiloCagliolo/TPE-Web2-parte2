...
----------------------------------------------------REQUESTS------------------------------------------------------------
--------GET-------
Una request GET .../api/exoplanets dará como resultado TODOS los exoplanetas de la tabla. Equivalentemente, una request GET .../api/stars dará TODAS las estrellas de la tabla.

Una request GET .../api/exoplanet/:ID donde ":ID" es un número, dará como resultado el exoplaneta asociado a esa id. Similarmente GET .../api/star/:ID dará la star asociada a esa ID.
...
----------Sobre la página:
En total hay SEIS MVC (Uno en realidad es solo VC porque no tiene model). Cada una de las tablas es operada por un Model y tiene su propio Controller y View, SALVO el model para methods que es operado por el NavController. NavController es el controller que se encarga únicamente de la navegación por la página y hace uso de ese model en particular porque lo necesita para renderizar cierto contenido, pero nada más: el contenido de methods no es editable de ninguna forma. MessageController es simplemente un controlador sin model para manejar las páginas de error (por ejemplo, al querer entrar a páginas restringidas o que no existen) y mensajes HTTP para que JavaScript los renderice en la página. (Ese controller fue sugerencia de Nicolás).

----------Explicación de la estructura de los archivos: 
En la carpeta "src" se encuentran todos los Models, Views y Controllers que corresponden a la página web. 
ParentView es una clase padre para todas las otras Views. 
...
Siempre que fue posible abstraer métodos y atributos repetidos a clases padres, traté de hacerlo. 
...
En la carpeta "api" se encuentran los controller y views para operar las tablas de Exoplanets y Stars mediante API REST. Estos controllers hacen uso de los models en la carpeta src, es decir, los que hice para la página. A diferencia de la página, que hace un control de sesión, la API permite acceder a la edición de las tablas sin pedir ninguna credencial.